...
function TestWaveMachine() {
  ...
  b1.SetAsBoxXYCenterAngle(0.03, 1, new b2Vec2(3, 0), 0);
  body.CreateFixtureFromShape(b1, 5);

  var b2 = new b2PolygonShape();
  b2.SetAsBoxXYCenterAngle(0.03, 1, new b2Vec2(-3, 0), 0);
  body.CreateFixtureFromShape(b2, 5);

  var b3 = new b2PolygonShape();
  b3.SetAsBoxXYCenterAngle(2, 0.03, new b2Vec2(0, 2), 0);
  body.CreateFixtureFromShape(b3, 5);

  var b4 = new b2PolygonShape();
  b4.SetAsBoxXYCenterAngle(3, 0.03, new b2Vec2(0, -1), 0);
  body.CreateFixtureFromShape(b4, 5);

  var jd = new b2RevoluteJointDef();
  jd.motorSpeed = 0.02 * Math.PI;
...
TestWaveMachine.prototype.Step = function () {
  ...
  this.time += 1 / 30;
  this.joint.SetMotorSpeed(0.02 * Math.sin(this.time) * Math.PI);
};
